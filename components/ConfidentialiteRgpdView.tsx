...
export function ConfidentialiteRgpdView({ locale }: ConfidentialiteRgpdViewProps) {
  const t = confidentialiteRgpdTranslations[locale];
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
      ...
      <p className="mt-14 rounded-2xl border border-[var(--border)] bg-[var(--muted)]/60 px-5 py-4 text-sm text-[var(--text-secondary)]">
        {t.contactHint}{" "}
        <Link href={`${basePath}/contact`} className="font-semibold text-[var(--navy-primary)] underline-offset-2 hover:underline">
          {locale === "fr" ? "Accéder au contact" : "Go to contact"}
        </Link>
      </p>
    </div>
  );
